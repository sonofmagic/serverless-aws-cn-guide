...
> 假如你有一个域名，你就可以把域名多配置一些主或多级域名，把它们的 `CNAME` 解析到指定的 `API网关` 地址来使用。当然你直接解析是不生效的，因为`API网关`有个双重验证，你必须进入`API网关`的 `自定义域名`界面，创建自定义域名，并绑定 `ACM 证书`，通过后才可以使用。
>
...
`serverless.yml` 里面声明的字符串也支持使用变量的方式来实现配置的引用与读取，它们是非常有用的，毕竟你不可能把某些配置项，诸如 `secret` 什么的直接 `inline` 写在 `yml` 文件里。
...
首先必须要讲的就是 `self` 了，它是我们应用配置自身其他值的关键。这里我给出一个示例，相信聪明的你可以一眼看出它的用法。
...
其中 `${sls:stage}` 指令的实质实际上是 `${opt:stage, self:provider.stage, "dev"}`
...
模块化配置的核心变量，使用这个变量方法可以去读取文件，并进行引用，例如我们可以在这里引入另外的 `yml`,`json`,`js`文件:
...
其中引入 `js` 文件的代码有一定的限制，它必须是导出一个`js`对象，或者是导出一个`function`，对象很简单且泛用性不强，我们这里以方法为例：
...
`serverless.js` 和 `nodejs` 运行时结合，也可以配合动态变量，更加灵活。

所以这里我们以 `serverless.js` 配置文件为例：

> 智能提示, 需要安装 `@serverless/typescript`
...
当然由于它是 `nodejs` 运行时的缘故，你可以把配置文件拆分成多个文件，再使用 `commonjs` 原生的方式去引用它们。你也可以使用环境变量，`node:fs`，`node:path`等等模块去按条件动态去生成配置文件，这实在是太灵活了！
